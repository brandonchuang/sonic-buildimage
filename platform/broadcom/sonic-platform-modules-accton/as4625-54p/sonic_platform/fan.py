...
FAN_HWMON_I2C_PATH = "/sys/devices/platform/as4625_54p_fan/hwmon/hwmon*/fan"
...
class Fan(FanBase):
    ...
    def get_speed(self):
        ...
        if self.is_psu_fan:
            ...
            max_path = "{}{}{}".format(self.hwmon_path, self.fan_tray_index+1, '_max')
            max = self.__read_txt_file(max_path)

            if input is None or max is None:
                return 0

            if int(max) > 0:
                speed = int(input) * 100 / int(max)
