 ...
 * This includes the:
 *	 Accton as4625_54p CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct as4625_54p_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id as4625_54p_cpld_id[] = {
    { "as4625_54p_cpld1", as4625_54p_cpld1 },
    { }
};
MODULE_DEVICE_TABLE(i2c, as4625_54p_cpld_id);
...
enum as4625_54p_cpld_sysfs_attributes {
	VERSION_MAJOR,
	VERSION_MINOR,
	ACCESS,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
             char *buf);
static int as4625_54p_cpld_read_internal(struct i2c_client *client, u8 reg);
static int as4625_54p_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *as4625_54p_cpld1_attributes[] = {
...
static const struct attribute_group as4625_54p_cpld1_group = {
	.attrs = as4625_54p_cpld1_attributes,
};
             ...
             char *buf)
{
    ...
    struct as4625_54p_cpld_data *data = i2c_get_clientdata(client);
	int status = 0;
	...
	status = as4625_54p_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
    ...
    status = as4625_54p_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0)) {
		goto exit;
	}
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = as4625_54p_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void as4625_54p_cpld_add_client(struct i2c_client *client)
{
...
static void as4625_54p_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int as4625_54p_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct as4625_54p_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct as4625_54p_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    switch (data->type) {
    case as4625_54p_cpld1:
        group = &as4625_54p_cpld1_group;
        break;
    default:
        break;
    }
    ...
    as4625_54p_cpld_add_client(client);
    return 0;
...
static int as4625_54p_cpld_remove(struct i2c_client *client)
{
    struct as4625_54p_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    as4625_54p_cpld_remove_client(client);
...
int as4625_54p_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as4625_54p_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(as4625_54p_cpld_read);

int as4625_54p_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as4625_54p_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(as4625_54p_cpld_write);

static struct i2c_driver as4625_54p_cpld_driver = {
	.driver		= {
		.name	= "as4625_54p_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= as4625_54p_cpld_probe,
	.remove		= as4625_54p_cpld_remove,
	.id_table	= as4625_54p_cpld_id,
};

static int __init as4625_54p_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&as4625_54p_cpld_driver);
}

static void __exit as4625_54p_cpld_exit(void)
{
    i2c_del_driver(&as4625_54p_cpld_driver);
}
...
module_init(as4625_54p_cpld_init);
module_exit(as4625_54p_cpld_exit);
